...
(() => {
  ...
  // Count letters from the saved calculation text. Keep the historical payload
  // unchanged; punctuation (including curly apostrophes) is not a letter.
  function countLetters(text){ return (text.match(/\p{L}/gu) || []).length; }
  function derivedLetterCount(verse){ return {reference:verse.reference, alphabetic_letters:countLetters(verse.calc_text), historical_recorded_letters:verse.calc_letter_count}; }
  ...
  let pointerId=null, pressX=0, pressY=0, suppressClick=false;
  function trackMovement(e){ if(Math.hypot(e.clientX-pressX,e.clientY-pressY)>5) suppressClick=true; }
  canvas.addEventListener('pointerdown', e=>{if(e.button!==0 || pointerId!==null) return; pointerId=e.pointerId; state.drag=true; suppressClick=false; pressX=state.lastX=e.clientX; pressY=state.lastY=e.clientY;});
  window.addEventListener('pointerup',e=>{if(e.pointerId!==pointerId) return; trackMovement(e); state.drag=false; pointerId=null;});
  window.addEventListener('pointercancel',e=>{if(e.pointerId!==pointerId) return; state.drag=false; pointerId=null; suppressClick=true;});
  window.addEventListener('pointermove',e=>{ if(state.drag && e.pointerId===pointerId){trackMovement(e); state.rotation += (e.clientX-state.lastX)*.007; state.lastX=e.clientX; state.lastY=e.clientY;}});
  ...
  canvas.addEventListener('click', e=>{ if(state.drag || suppressClick){suppressClick=false; return;} const rect=canvas.getBoundingClientRect(); const x=e.clientX-rect.left,y=e.clientY-rect.top; const w=rect.width,h=rect.height,cx=w/2,cy=h/2,base=Math.min(w,h)*.35*state.zoom; let best=-1,bd=9999; DATA.qmap_nodes.forEach((n,i)=>{const day=(nodeVerses(n)[0]?.day)||0; const ring=(.28+day*.08)*base; const angle=state.rotation+(i/DATA.qmap_nodes.length)*Math.PI*2-Math.PI/2; const px=cx+Math.cos(angle)*ring; const py=cy+Math.sin(angle)*ring*.72; const d=Math.hypot(px-x,py-y); if(d<bd){bd=d;best=i;}}); if(bd<42) selectNode(best); });
  function renderMetrics(){
    const families=DATA.whole_word_instrument_research_v4||[];
    const g={...DATA.global_metrics,
      core_calc_letters:DATA.core_verses.reduce((total,v)=>total+countLetters(v.calc_text),0),
      instrument_families_with_recorded_matches:families.filter(f=>f.match_count>0).length,
      instrument_families_configured:families.length};
    $('#globalMetrics').innerHTML=['core_verse_count','core_calc_words','core_calc_letters','core_gematria_mass','core_raw_bits','core_character_events','core_note_events','core_ornament_events','witness_group_count','unique_witness_reference_count','instrument_families_with_recorded_matches','instrument_families_configured'].map(k=>`<div class="metric"><span>${k.replaceAll('_',' ')}</span><b>${g[k]}</b></div>`).join('');
  }
 ...
 ev.appendChild(it); }); }
  ...
  function exportSelection(){ const verses=nodeVerses(getSelectedNode()); const payload={node:getSelectedNode(), verses, selected_word:getSelectedWord(), derived_letter_counts:{method:'Unicode letters in saved calc_text; punctuation and spaces excluded', verses:verses.map(derivedLetterCount), selection_letters:verses.reduce((total,v)=>total+countLetters(v.calc_text),0)}, historical_data_notice:'verses preserves historical source fields; use derived_letter_counts for corrected alphabetic counts.'}; const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='q6_selected_qmap_node.json'; a.click(); URL.revokeObjectURL(a.href); }
