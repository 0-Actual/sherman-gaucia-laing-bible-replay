...
(() => {
  ...
  const pitchColors = {"C":"#87e8ff","C#":"#78fff5","D":"#80ffb6","D#":"#b7ff6f","E":"#fff27a","F":"#ffd36e","F#":"#ffac6e","G":"#ff7a9e","G#":"#d988ff","A":"#a88bff","A#":"#7aa5ff","B":"#8fffd9"};
  ...
  function ensureAudio(){
    ...
    const AC = window.AudioContext || window.webkitAudioContext; state.audio = new AC(); state.master = state.audio.createGain(); state.master.gain.value = .55; state.master.connect(state.audio.destination); return state.audio;
  }
  function envGain(ac, t, a=.01, d=.2, peak=.22){ const g=ac.createGain(); g.gain.setValueAtTime(0.0001,t); g.gain.exponentialRampToValueAtTime(Math.max(0.001,peak),t+a); g.gain.exponentialRampToValueAtTime(0.0001,t+a+d); g.connect(state.master); return g; }
  function noiseBuffer(ac, len=0.4){ const b=ac.createBuffer(1, Math.floor(ac.sampleRate*len), ac.sampleRate); const ch=b.getChannelData(0); for(let i=0;i<ch.length;i++) ch[i]=Math.random()*2-1; return b; }
  function playTone(freq=220, instrument='harp_letter', dur=.25, when=null, velocity=.65){
    const ac=ensureAudio(); const t = when ?? ac.currentTime; instrument=instFamily(instrument); if(state.muted[instrument]) return;
    dur=Math.max(.04, Math.min(dur, 2.5)); freq=Math.max(30, Math.min(freq||220, 3000));
    if(instrument==='tambourine_attack' || instrument==='cymbal_cadence'){
      const src=ac.createBufferSource(); src.buffer=noiseBuffer(ac, instrument==='cymbal_cadence'?1.2:.22);
      const filter=ac.createBiquadFilter(); filter.type='bandpass'; filter.frequency.value=instrument==='cymbal_cadence'?6200:2200; filter.Q.value=instrument==='cymbal_cadence'?0.7:2.8;
      const g=envGain(ac,t,.004,instrument==='cymbal_cadence'?1.0:.12,velocity*.24); src.connect(filter); filter.connect(g); src.start(t); src.stop(t+dur+1); return;
    }
    if(instrument==='water_bowl'){
      const osc=ac.createOscillator(); const filter=ac.createBiquadFilter(); filter.type='lowpass'; filter.frequency.value=600; const g=envGain(ac,t,.12,dur*1.3,velocity*.14); osc.type='sine'; osc.frequency.setValueAtTime(freq/2,t); osc.frequency.exponentialRampToValueAtTime(freq*0.75,t+dur); osc.connect(filter); filter.connect(g); osc.start(t); osc.stop(t+dur+1); return;
    }
    const osc=ac.createOscillator(); const g=envGain(ac,t, instrument.includes('voice')||instrument.includes('strings')||instrument.includes('choir')?.08:.01, dur, velocity*.22);
    if(instrument==='harp_letter'){osc.type='triangle'}
    else if(instrument==='flute_vowel_color'){osc.type='sine'}
    else if(instrument==='pipe_ornament'){osc.type='sine'}
    else if(instrument==='trumpet_luminary'||instrument==='shofar_trumpet'){osc.type='sawtooth'}
    else if(instrument==='lyre_witness'){osc.type='triangle'}
    else if(instrument==='strings_field'||instrument==='choir_rest'||instrument==='voice_cantillation'){osc.type='sine'}
    else {osc.type='sine'}
    osc.frequency.setValueAtTime(freq,t);
    if(instrument==='shofar_trumpet'){ osc.frequency.linearRampToValueAtTime(freq*1.25,t+Math.min(.3,dur)); }
    if(instrument==='voice_cantillation'||instrument==='flute_vowel_color'){ const vib=ac.createOscillator(); const vg=ac.createGain(); vib.frequency.value=5.2; vg.gain.value=freq*.006; vib.connect(vg); vg.connect(osc.frequency); vib.start(t); vib.stop(t+dur+.2); }
    osc.connect(g); osc.start(t); osc.stop(t+dur+.3);
    if(instrument==='strings_field'||instrument==='choir_rest'){
      const osc2=ac.createOscillator(); const g2=envGain(ac,t,.12,dur*1.1,velocity*.09); osc2.type='sawtooth'; osc2.frequency.setValueAtTime(freq*1.005,t); osc2.connect(g2); osc2.start(t); osc2.stop(t+dur+.5);
    }
  }
  function playEvents(events, maxCount=2200){
    ensureAudio(); stopAudio(); state.playing=true; $('#playBtn').classList.add('active');
    let t=state.audio.currentTime+.08; const scale=Number($('#speed').value||1); let count=0;
    for(const ev of events){
      if(count++>maxCount) break;
      if(ev.event_kind==='rest'){ t += .06/scale; continue; }
      const dur=(Number(ev.duration_units)||1)*.055/scale;
      playTone(ev.frequency_hz || 220, ev.instrument || 'harp_letter', dur, t, ev.event_kind==='ornament'?.38:.72);
      t += Math.max(.025, dur*(ev.event_kind==='ornament'?.45:.8));
    }
    setTimeout(()=>{state.playing=false; $('#playBtn').classList.remove('active');}, Math.max(100,(t-state.audio.currentTime)*1000));
  }
  function stopAudio(){ if(state.scheduled.length){state.scheduled.forEach(x=>{try{x.stop()}catch(e){}}); state.scheduled=[];} }
  ...
  function renderRefs(){ const box=$('#refImages'); box.innerHTML='<img src="assets/REFERENCE_IMAGE_HAND_FREQUENCY_QMAP.jpg" alt="hand frequency QMAP"><img src="assets/REFERENCE_IMAGE_QUANTUM_MAP_WORLDFIELD.jpg" alt="quantum map worldfield"><img src="assets/REFERENCE_IMAGE_OPENAI_ARTICLE_SCREENSHOT.jpg" alt="OpenAI article reference">'; }
